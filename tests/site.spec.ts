...
test('対応タグは否定・推測・伝聞の記載を拾わない', async ({ page }) => {
  // ノリス美容クリニック：「豊胸術を行わずに女性化」＝豊胸は“しない”文脈なので豊胸タグは付かない。
  // 一方「精巣切除（摘除）の手術も相談可能」は肯定なので睾丸摘出術タグは付く。
  await page.goto('/clinics/hrt-osaka-norris/');
  const norrisTags = await textNoRuby(page.locator('.clinic-tags'));
  expect(norrisTags).not.toContain('豊胸');
  expect(norrisTags).toContain('睾丸摘出');

  // 横浜中央クリニック：「女性ホルモン注射もやっていると考えられる」＝推測なのでMtFタグは付かない。
  await page.goto('/clinics/hrt-kanagawa-yokohama-chuuou/');
  const yokoTags = page.locator('.clinic-tags');
  if (await yokoTags.count()) {
    expect(await textNoRuby(yokoTags)).not.toContain('女性化ホルモン');
  }

  // ユリシスクリニック：「除睾術にも対応」＝睾丸摘出術タグが付く（取りこぼさない）。
  await page.goto('/clinics/hrt-shizuoka-ulysses/');
  expect(await textNoRuby(page.locator('.clinic-tags'))).toContain('睾丸摘出');

  // 肯定的に豊胸を掲載している施設では、これまでどおりタグが付く。
  await page.goto('/clinics/hrt-osaka-nagumo/');
  expect(await textNoRuby(page.locator('.clinic-tags'))).toContain('豊胸');
});

test('医療機関ページからタグ（テーマ別の地図）へたどれる', async ({ page }) => {
  // クリニック詳細から、診療区分に対応するタグ（/tags/）へのリンクがある。
  await page.goto('/clinics/hrt-tokyo-gender-clinic/');
  await expect(page.locator('.page-tags a[href="/tags/hormones/"]')).toBeVisible();

  // 資料・リンクのハブにも「タグで探す」がある。
  await page.goto('/reference/');
  await expect(page.locator('.topic-item[href="/tags/"]')).toBeVisible();
});
...
test('医療機関一覧を「国内・海外」で絞り込め、海外の手術病院が一覧に出る', async ({ page }) => {
  await page.goto('/clinics/');

  // 国内・海外フィルターの次元がある。
  const scope = page.locator('[data-filter-tabs][data-filter-key="scope"]');
  await expect(scope).toBeVisible();

  // 「海外」を選ぶと、海外の手術病院（タイのガモン病院）が出て、国内クリニックは隠れる。
  await scope.getByRole('button', { name: '海外', exact: true }).click();
  await expect(page.locator('.clinic-name-link', { hasText: 'ガモン病院（Kamol）' })).toBeVisible();
  await expect(page.locator('.clinic-name-link', { hasText: '東京ジェンダークリニック' })).toBeHidden();
});

test('手術ページに海外（タイ・韓国）の病院セクションとFFSの施設がある', async ({ page }) => {
  await page.goto('/clinics/surgery/');

  // 海外セクションと、その中のタイの病院（本文は furigana ruby を含むので href で判定）。
  const overseas = page.locator('section[aria-labelledby="overseas"]');
  await expect(overseas).toBeVisible();
  await expect(overseas.locator('a[href="/clinics/srs-overseas-th-pai/"]')).toBeVisible();

  // FFS専門の国内クリニックが手術一覧に出る。
  await expect(page.locator('a[href="/clinics/ffs-ishikawa-kanazawa/"]')).toBeVisible();
});

test('海外の医療機関ページが国名つきで表示される', async ({ page }) => {
  await page.goto('/clinics/srs-overseas-th-kamol/');
  // タイトルに国名（タイ）が入る。
  await expect(page).toHaveTitle(/タイ/);
  // 本文で海外であることがわかる（ruby を除いて判定）。
  const body = await textNoRuby(page.locator('main'));
  expect(body).toContain('海外');
});
...
test('歴史の年表は人名や出来事を百科事典（ウィキペディア等）へリンクする', async ({ page }) => {
  await page.goto('/history/');

  // 年表の各出来事から、外部の百科事典へ新しいタブで安全に開くリンクが多数ある。
  const wikiLinks = page.locator(
    '.timeline-event a[href*="wikipedia.org"][target="_blank"]',
  );
  expect(await wikiLinks.count()).toBeGreaterThanOrEqual(30);

  // 外部リンクは rel="noreferrer" を付けて開く。
  await expect(wikiLinks.first()).toHaveAttribute('rel', /noreferrer/);

  // 代表的な項目: ストーンウォールの反乱は日本語版ウィキペディアへ。
  await expect(
    page.locator('.timeline-event a[href*="ja.wikipedia.org"]').first(),
  ).toBeVisible();
});

test('学校ページは具体的な場面と、文科省通知（配慮の根拠）を示す', async ({ page }) => {
  await page.goto('/school/');

  // 学校生活の具体的な場面がセクションとして並ぶ。
  await expect(page.locator('section[aria-labelledby="uniform"]')).toBeVisible();
  await expect(page.locator('section[aria-labelledby="trip"]')).toBeVisible();
  await expect(page.locator('section[aria-labelledby="bully"]')).toBeVisible();

  // 配慮の根拠として文部科学省の通知へリンクする（先生に見せられる）。
  await expect(
    page.locator('a[href="https://www.mext.go.jp/b_menu/houdou/27/04/1357468.htm"]').first(),
  ).toBeVisible();
});

test('よくある誤解ページは「誤解／ほんとうは」を出典つきで並べる', async ({ page }) => {
  await page.goto('/myths/');

  // 誤解と事実のラベルが対で並ぶ。
  await expect(page.locator('.myth-tag-no').first()).toBeVisible();
  await expect(page.locator('.myth-tag-yes').first()).toBeVisible();

  // 事実に出典マーカー（[n]）が付いている。
  await expect(page.locator('.myth-truth .cite').first()).toBeVisible();

  // くわしく読むための内部リンク（深いページへの導線）がある。
  await expect(page.locator('.myth-item a[href="/detransition/"]').first()).toBeVisible();

  // 参考文献リストが出る。
  await expect(page.locator('[id^="ref-myths-"]').first()).toBeVisible();
});

test('一人称・呼ばれ方ページは一人称の例と、出典つきの呼び名の話を示す', async ({ page }) => {
  await page.goto('/pronouns/');

  // 一人称の例（僕・私など）のセクションがある。
  await expect(page.locator('section[aria-labelledby="first-person"]')).toBeVisible();

  // 呼ばれ方の話に出典マーカー（[n]）が付いている。
  await expect(page.locator('.cited .cite').first()).toBeVisible();

  // 名前・改名の深いページへの導線がある。
  await expect(page.locator('.entry-related a[href="/legal-change/"]').first()).toBeVisible();

  // 参考文献リストが出る。
  await expect(page.locator('[id^="ref-pronouns-"]').first()).toBeVisible();
});

test('保護者ページは受容の根拠（出典つき）と親自身の相談先を示す', async ({ page }) => {
  await page.goto('/parents/');

  // 受けとめが子を守る、という根拠のセクションがある。
  await expect(page.locator('section[aria-labelledby="evidence"]')).toBeVisible();

  // 本文に出典マーカー（[n]）が付いている。
  await expect(page.locator('.cited .cite').first()).toBeVisible();

  // 親・家族のための相談先（つなぐ会）へリンクする。
  await expect(
    page.locator('a[href="https://lgbt-family.or.jp/"]').first(),
  ).toBeVisible();

  // 当事者向けの周囲ページと相互にリンクする。
  await expect(page.locator('.entry-related a[href="/allies/"]').first()).toBeVisible();

  // 参考文献リストが出る。
  await expect(page.locator('#ref-parents-1, [id^="ref-parents-"]').first()).toBeVisible();
});

test('資料集（取り込み記事）の表が枠線付きの表として表示される', async ({ page }) => {
  await page.goto('/library/mtf-wiki-ja/docs/medicine/risk/');
  const cell = page.locator('.prose table td').first();
  await expect(cell).toBeVisible();
  // マークダウンの表に枠線・余白が当たっている（素のままではない）。
  const border = await cell.evaluate((el) => parseFloat(getComputedStyle(el).borderBottomWidth));
  expect(border).toBeGreaterThan(0);
});

test('主要ページはそれぞれ専用の OG カードを持つ（既定画像のフォールバックではない）', async ({ page }) => {
  for (const [path, slug] of [['/school/', 'school'], ['/history/', 'history'], ['/tags/', 'tags']]) {
    await page.goto(path);
  ...
  // コレクションの個別ページ（用語・タグなど）も、索引カードの共有ではなく
  // それぞれ自分の名前が入った専用カードを持つ。
  await page.goto('/tags/identity/');
  await expect(page.locator('meta[property="og:image"]')).toHaveAttribute('content', /\/og\/tags-identity\.png$/);
  await page.goto('/glossary/non-binary/');
  await expect(page.locator('meta[property="og:image"]')).toHaveAttribute(
    'content',
    /\/og\/glossary-non-binary\.png$/,
  );
});

test('用語ページは手作業で選んだ「関連することば」を用語どうしで相互リンクする', async ({ page }) => {
  await page.goto('/glossary/transgender/');
  const related = page.locator('nav[aria-label="関連する用語"]');
  await expect(related).toBeVisible();
  // 対になる語（シスジェンダー）への用語間リンクがある。
  await expect(related.locator('a[href="/glossary/cisgender/"]')).toBeVisible();
  // 逆方向も: シスジェンダー側からトランスジェンダーへ戻れる。
  await page.goto('/glossary/cisgender/');
  await expect(
    page.locator('nav[aria-label="関連する用語"] a[href="/glossary/transgender/"]'),
  ).toBeVisible();
});

test('歴史ページは世界の文化と東アジアの歩みを含み、西洋中心でない', async ({ page }) => {
  await page.goto('/history/');

  // 地域ごとの「世界の文化のなかの多様な性」と「東アジアの歩み」のセクションがある。
  await expect(page.locator('section[aria-labelledby="cultures"]')).toBeVisible();
  await expect(page.locator('section[aria-labelledby="east-asia"]')).toBeVisible();

  // 各地の用語・人物・制度が百科事典に多数リンクされている（世界各地に分散）。
  const wiki = page.locator('.prose a[href*="wikipedia.org"][target="_blank"]');
  expect(await wiki.count()).toBeGreaterThanOrEqual(50);

  // 文化セクションに非西洋の項目リンクがある（例: ムシェ＝メキシコ・サポテク）。
  await expect(
    page.locator('section[aria-labelledby="cultures"] a[href*="/Muxe"]'),
  ).toHaveCount(1);
});

test('知る・学ぶは記事をテーマ別に分け、タグと主要ページへの導線を持つ', async ({ page }) => {
  await page.goto('/learn/');

  // テーマごとのセクション（見出しは furigana ruby を含むので aria-label で判定）。
  await expect(page.locator('section[aria-label="からだと医療"]')).toBeVisible();
  await expect(page.locator('section[aria-label="調べる・参考資料"]')).toBeVisible();

  // 以前は載っていなかった主要ページへの導線がカードとしてある。
  for (const href of ['/map/', '/resources/', '/library/', '/reference/']) {
    await expect(page.locator(`.topic-item[href="${href}"]`)).toBeVisible();
  }
});

test('プライドページは Tokyo Pride の公式サイト（pride.tokyo）へリンクする', async ({ page }) => {
  await page.goto('/pride/');
  const link = page.locator('a[href="https://pride.tokyo/"]').first();
  await expect(link).toBeVisible();
  await expect(link).toHaveAttribute('rel', /noreferrer/);
});

test('イベントバナーの一回性イベントは年指定で、定例より先に評価される', async ({ page }) => {
  await page.goto('/');
  const data = await page.locator('#event-banner-data').textContent();
  const events = JSON.parse(data ?? '[]') as Array<{ id: string; year?: number }>;

  const tp = events.findIndex((e) => e.id === 'tokyo-pride-2026');
  const month = events.findIndex((e) => e.id === 'pride-month');
  expect(tp).toBeGreaterThanOrEqual(0);
  // 一回性イベントは year を持ち、最初に一致するよう定例（プライド月間）より前に並ぶ。
  expect(events[tp].year).toBe(2026);
  expect(tp).toBeLessThan(month);
});
